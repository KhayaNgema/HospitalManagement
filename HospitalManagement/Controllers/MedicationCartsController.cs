...
{
    public class MedicationCartController : Controller
    {
        private readonly HospitalManagementDbContext _context;
        private readonly IHttpContextAccessor _httpContextAccessor;

        public MedicationCartController(HospitalManagementDbContext context,
        ...
        {
            var menuItem = _context.MedicationStocks.FirstOrDefault(m => m.StockId == menuItemId);
            ...
            if (menuItem != null)
            {
                ...
                var existingCartItem = cart.Items.FirstOrDefault(ci => ci.MedicationStock.Medication.MedicationId == menuItemId);
        ...
        private MedicationCart GetCart()
        {
            var cart = HttpContext.Session.GetObject<MedicationCart>("Cart");
        ...
        private void SaveCart(MedicationCart cart)
        {
            HttpContext.Session.SetObject("Cart", cart);
        }

        private string RenderCartItemsHtml(List<MedicationCartItem> cartItems)
        {
            ...
            if (cartItems != null && cartItems.Any())
            {
                ...
                {
                    var menuItem = _context.MedicationStocks.FirstOrDefault(m => m.StockId == cartItem.MedicationStock.Medication.MedicationId);

                    if (menuItem != null)
                    {
                        htmlBuilder.Append("<tr>");
                        htmlBuilder.Append($"<td>{menuItem.Medication.MedicationName}</td>");
